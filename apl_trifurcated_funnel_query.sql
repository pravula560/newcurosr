...
-- Optional SQL filter: flag_eligible_lead = TRUE via __FLAG_ELIGIBLE_SQL__ (see run_apl_trifurcated_funnel.py --eligible-only).
...
--
-- Placeholders: __PRODUCT_LINE_IN__, __CREATED_START__, __CREATED_END_SQL__, __FLAG_ELIGIBLE_SQL__
...
WITH base AS (
  ...
  WHERE UPPER(TRIM(COALESCE(A.product_line, ''))) IN (__PRODUCT_LINE_IN__)
    AND DATE(A.created_datetime) >= '__CREATED_START__'
__CREATED_END_SQL__
