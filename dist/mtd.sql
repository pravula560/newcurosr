-- APL trifurcated funnel dashboard extract (Core / Pagaya / Superprime).
-- Optional SQL filter: flag_eligible_lead = TRUE via  (see run_apl_trifurcated_funnel.py --eligible-only).
-- Default extract includes all eligibility values so the dashboard "flag_eligible_lead only" checkbox can filter client-side.
-- Cohort date on created_datetime: DATE(created_datetime) >= start;
-- optional upper bound via run_apl_trifurcated_funnel.py --end-date or --mtd (omit end for open cohort).
--
-- Dashboard / BigQuery validation alignment:
--   LOANS  = COUNT(DISTINCT application_key) by product_line
--   FUNDS  = COUNT(DISTINCT loan_id) WHERE funding_datetime IS NOT NULL
-- The funded / origination_dollars columns below use funding_datetime only (not funded_date).
--
-- Includes FICO bands, Pagaya PQ vs FA cohort flags, and pagaya_entry_segment for View 2.
--
-- Requires on fplus_application: pq_product_prioritization, fa_product_prioritization,
--   pq_pagaya_evaluated, pq_pagaya_approved, pagaya_offers_requested (same as pagaya_pq_fa_merged_query.sql).
--
-- Placeholders: 'CORE', 'PAGAYA', 'SUPERPRIME', 2026-03-01,     AND DATE(A.created_datetime) <= '2026-03-15', 
WITH base AS (
  SELECT
    A.application_key,
    A.loan_id,
  ...
  WHERE UPPER(TRIM(COALESCE(A.product_line, ''))) IN ('CORE', 'PAGAYA', 'SUPERPRIME')
    AND DATE(A.created_datetime) >= '2026-03-01'
    AND DATE(A.created_datetime) <= '2026-03-15'
